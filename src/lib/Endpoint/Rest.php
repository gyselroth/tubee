...
use Tubee\Endpoint\Rest\Exception;
use Tubee\EndpointObject\EndpointObjectInterface;
use Tubee\Workflow\Factory as WorkflowFactory;

class Rest extends AbstractEndpoint
{
    /**
     * Guzzle client.
     *
    ...
    {
        $this->logger->info('update object on endpoint ['.$this->getIdentifier().'] using ['.$this->update_method.'] to ['.$this->client->getConfig('base_uri').'/'.$this->getResourceId($endpoint_object).']', [
            'category' => get_class($this),
        ]);

        if ($simulate === false) {
            if ($this->update_method === 'PATCH') {
                $result = $this->client->patch('/'.$this->getResourceId($endpoint_object), [
                    'json' => $diff,
                ]);
            } else {
                $result = $this->client->put('/'.$this->getResourceId($endpoint_object), [
                    'json' => $object,
                ]);
            }
        }
    ...
    {
        $this->logger->debug('find all rest objects using ['.$this->client->getConfig('base_uri').']', [
            'category' => get_class($this),
        ]);
